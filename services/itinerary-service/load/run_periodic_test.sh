#!/bin/bash
# Script to run the periodic workload test
...
echo "========================================="
echo "🔄 Periodic Workload Load Test"
echo "========================================="
echo ""

# Check if .env.periodic exists
if [ ! -f ".env.periodic" ]; then
    echo "⚠️  .env.periodic not found!"
    echo "Creating from config_periodic.txt..."
    cp config_periodic.txt .env.periodic
    echo "✅ Created .env.periodic - please review and adjust if needed"
    echo ""
fi
...
REPORT_NAME="periodic_test_$(date +%Y%m%d_%H%M%S).html"
...
if [ "$1" == "--headless" ] || [ "$1" == "-h" ]; then
    ...
    echo "4. Download the report when finished"
    echo "========================================="
    echo ""
